...
# Model configuration for DiffuCoder (Base Model for No-Prompt Testing)
MODEL_NAME = "apple/DiffuCoder-7B-Base"
MASK_TOKEN = "<|mask|>"
NUM_MASK_TOKENS = 4  # Typical number of tokens for an identifier
...
def extract_prediction(full_code, masked_code):
    ...
    # This correctly handles "queryBuilder" even if it was 2 tokens
    # and ignores trailing " ) )" hallucinations
...
def run_experiment():
    ...
    print(f"\n{'='*50}")
    print(f"Running No-Prompt Experiment for: DiffuCoder Base")
    ...
    # Process samples
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Testing DiffuCoder Base"):
        item_id = row['id']
        masked_code = str(row['masked_code'])
        ground_truth = str(row['target']).strip()

        try:
            # CRITICAL FIX: Do NOT use spaces between mask tokens.
            # This allows the model to predict a single long identifier across tokens.
            ...
            # Optimization: Reduced steps from 256 to 32.
            # For identifier in-filling, 32-64 steps is usually enough and much faster.
            GEN_STEPS = 32
    ...
    output_file = os.path.join(RESULTS_DIR, f"DiffuCoder_Base_noPrompt_{timestamp}.csv")
