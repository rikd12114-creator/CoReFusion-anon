"""
LLM-as-a-judge evaluation for the CodeT5 family on the RefineID benchmark.

Implements the setup documented in Appendix `app:llm_judge` of the paper:
    * Judge model:  Qwen/Qwen2.5-7B-Instruct (open weights)
    * Backend:      HuggingFace Transformers, bfloat16, single GPU
    * Sampling:     temperature=0, top_p=1.0, max_new_tokens=200
    * Off-the-shelf, no fine-tuning or quantisation
    * Prompt:       the verbatim template from app:llm_judge_prompt
    * Score:        LJ is the fraction of rows whose `Decision` field == Accept.
                    Anything else (Reject / unparseable / generation failure)
                    counts as a rejection.

The script reproduces the exact judge configuration used to score the AR-FIM
and dLLM rows of Table~\\ref{tab:models}, so the resulting CodeT5 numbers can
be filled into the same column without methodological drift.
...
Usage
-----
    # Score all five CodeT5 result CSVs from the FIM benchmark directory
    python detector/llm_naming_judge_codet5.py \
        --fim-dir /path/to/ar_fim_benchmark \
        --test-data data/test.csv \
        --output-dir results/llm_judge_codet5

    # Or score a specific file
    python detector/llm_naming_judge_codet5.py \
        --result-file results/ar_fim_benchmark/CodeT5-large_refineID_fim_20260428_172907.csv \
        --test-data data/test.csv

    # Quick smoke-test with 20 rows per file
    python detector/llm_naming_judge_codet5.py --fim-dir ... --limit 20
"""
...
# CodeT5 model names recognised in the FIM benchmark directory.
CODET5_TAGS = ("CodeT5-small", "CodeT5-base", "CodeT5-large",
...
# A code-context truncation budget keeps prompts under the judge's input
# window when a snippet is very long. Same window the original AR/dLLM
# judge run used; mirrors the constant in the legacy gpt-4o-mini script.
...
def parse_judge_output(text):
    """Extract (relationship, explanation, decision) from the judge's reply.

    Per the appendix, anything other than an explicit `Accept` is treated as
    a rejection. Unparseable outputs therefore fail closed.
    """
...
def collect_codet5_files(args):
    """Resolve the list of result CSVs to score."""
    files = []
    if args.result_file:
        files.extend(args.result_file)
    if args.fim_dir:
        for path in sorted(glob.glob(os.path.join(args.fim_dir, "*.csv"))):
            base = os.path.basename(path)
            if base.startswith("summary_"):
                continue
            if any(tag in base for tag in CODET5_TAGS):
                files.append(path)
